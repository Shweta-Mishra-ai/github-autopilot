...
PATTERNS = [
    ...
    ("Generic API Key",      r"(?i)(api[_-]?key|apikey|api[_-]?secret).{0,10}['\"][a-zA-Z0-9_\-]{20,}['\"]"),
    ("Generic Password",     r"(?i)(password|passwd|pwd).{0,5}[=:].{0,5}['\"][^'\"]{8,}['\"]"),
    ("Generic Token",        r"(?i)(token|secret).{0,10}[=:].{0,5}['\"][a-zA-Z0-9_\-\.]{20,}['\"]"),
...
def format_findings(findings: list[SecretFinding], repo: str) -> str:
    ...
    for f in findings:
        lines.append(f"| {f.line_number} | {f.pattern_name} | `{f.severity}` | `{f.redacted_match}` |")
