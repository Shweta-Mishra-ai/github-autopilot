...
def run_stale_check(repo: str, installation_id: int):
    ...
    try:
        ...
        issues = gh_get(
            f"/repos/{repo}/issues?state=open&per_page=50&sort=updated&direction=asc", token
        )
...
def _mark_stale(repo: str, issue: dict, token: str, config):
    ...
    days_inactive = (
        datetime.utcnow() - datetime.strptime(issue["updated_at"], "%Y-%m-%dT%H:%M:%SZ")
    ).days
    ...
    if auto_close:
        try:
            ...
            log.info(
                "schedule.stale_auto_closed", repo=repo, issue=issue_number, days=days_inactive
            )
            ...
            log.error("schedule.stale_close_failed", repo=repo, issue=issue_number, error=str(e))
        return
...
"""
    ...
    try:
        ...
        gh_post(f"/repos/{repo}/issues/{issue_number}/labels", token, {"labels": ["stale"]})
        gh_post(
            f"/repos/{repo}/issues/{issue_number}/comments",
            token,
            {"body": comment + config.footer},
        )
        log.info("schedule.stale_marked", repo=repo, issue=issue_number, title=title[:50])
    except GitHubError as e:
        log.error("schedule.stale_mark_failed", repo=repo, issue=issue_number, error=str(e))
