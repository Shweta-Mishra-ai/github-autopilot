...
"""
    ...
    try:
        ...
        gh_post(
            f"/repos/{repo}/issues/{issue_number}/labels", token, {"labels": ["stale"]}
        )
        gh_post(
            f"/repos/{repo}/issues/{issue_number}/comments",
            token,
        ...
        log.info(
            "schedule.stale_marked", repo=repo, issue=issue_number, title=title[:50]
        )
    except GitHubError as e:
        log.error(
            "schedule.stale_mark_failed", repo=repo, issue=issue_number, error=str(e)
        )
