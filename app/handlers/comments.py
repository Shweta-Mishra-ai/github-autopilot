...
def handle(payload: dict):
    ...
    if response:
        ...
        try:
            gh_post(
                f"/repos/{repo}/issues/{issue_number}/comments", token, {"body": full}
            )
...
def _cmd_apply(
    repo: str, issue_number: int, ctx_title: str, context: str, token: str
) -> str:
    try:
    ...
    {{"sha": "full_sha", "old_message": "original", "new_message": "conventional: message"}}
        ...
        if fixed:
            try:
                ...
                gh_patch(
                    f"/repos/{repo}/git/refs/heads/{fix_branch}",
                    token,
                    {"sha": last_sha},
                )
            except Exception as e:
...
{{
  ...
  "refactors": [{{"type": "extract_function", "description": "what and why", "before": "snippet", "after": "refactored", "benefit": "benefit"}}]
    ...
    for i, ref in enumerate(r.get("refactors", [])[:4], 1):
        lines.append(
            f"### {i}. `{ref.get('type', '').upper()}` — {ref.get('description', '')}"
        )
        if ref.get("before"):
...
def _cmd_summarize(repo: str, issue_number: int, token: str) -> str:
    try:
        ...
        thread = "\n\n".join(
            f"@{c['user']['login']}: {c['body'][:300]}" for c in comments[:20]
        )
...
def _cmd_changelog(repo: str, token: str) -> str:
    try:
        ...
        commit_list = "\n".join(
            f"- {c['commit']['message'].split(chr(10))[0]}" for c in commits[:15]
        )
...
def _cmd_rollback(
    repo: str, issue_number: int, token: str, cmd_args: str, author: str
) -> str:
    """
        ...
        try:
            if action_type == "create_issue":
                ...
                if number:
                    ...
                    restored.append(
                        f"Closed issue #{number}: {action.get('title', '')[:50]}"
                    )
                ...
                if number and labels:
                    for lbl in labels:
                        try:
                            ...
                            gh_delete(
                                f"/repos/{repo}/issues/{number}/labels/{lbl}", token
                            )
                        except Exception:
                            pass
            ...
            failed.append(
                f"{action_type} #{action.get('number', '?')}: {str(exc)[:60]}"
            )
