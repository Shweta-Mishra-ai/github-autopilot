    ...
    try:
        ...
        if result.get("merged"):
            ...
            try:
                ...
                _r().lpush("audit:merge", _j.dumps({
                    "repo": repo, "pr": issue_number, "by": author,
                    "at": int(_t.time()), "sha": result.get("sha", "")[:12],
                }))
...
def cmd_apply(
    ...
    try:
        ...
        existing = gh_get(
            f"/repos/{repo}/pulls?head={owner}:{branch}&state=open&per_page=5", token
        )
        ...
        return (
            ...
            f"**Branch:** `{branch}` → `{default_branch}`\n\n"
            f"> Review changes carefully before merging."
        )
...
def cmd_rollback(
    ...
    Two-step confirmation prevents accidental destructive rollbacks.
    Safety snapshot taken BEFORE rollback; abort if it fails.
    """
    from app.core.snapshot import (
        get_snapshot_by_number, format_snapshot_list,
        format_rollback_result, take_snapshot,
    )
    ...
    if not confirm:
        return (
            ...
            f"**Snapshot #{n}** — `{snap_ts}` trigger: `{snap.get('trigger','unknown')}`\n\n"
            f"**Actions to undo:**\n{action_lines}\n\n"
            f"{'*(and more...)*' if len(bot_actions) > 5 else ''}\n\n"
            f"**Proceed:** `/rollback {n} confirm`\n"
            f"**Cancel:** ignore this message"
        )
    ...
    try:
        ...
        test_workflow = next(
            (wf for wf in all_workflows
             if any(n in wf.get("path","").lower() or n in wf.get("name","").lower()
                    for n in TEST_NAMES)),
            None,
        )
...
def cmd_security(
    ...
    try:
        ...
        lines.append(fmt_secrets(all_findings, repo) if all_findings
                     else "✅ **No secrets detected** in changed files.\n")
        lines.append(format_dep_findings(dep_findings) if dep_findings
