...
class TestContextManager:
    ...
    def test_add_and_get_history(self):
        ...
        ctx = ContextManager("test/repo", 1)
        with patch("app.core.context_manager.ContextManager.add") as mock_add:
            ctx.add("fix", "Fixed null check")
            mock_add.assert_called_once()

    def test_get_history_empty_returns_empty_string(self):
        from app.core.context_manager import ContextManager
        ctx = ContextManager("test/repo", 999)
        with patch("app.core.context_manager.ContextManager.get_history", return_value=""):
            result = ctx.get_history()
        assert result == ""

    def test_get_commands_used_returns_list(self):
        from app.core.context_manager import ContextManager
        ctx = ContextManager("test/repo", 1)
        with patch("app.core.context_manager.ContextManager.get_commands_used", return_value=["fix", "explain"]):
            cmds = ctx.get_commands_used()
...
class TestSafeImport:
    ...
    def test_safe_import_invalid_returns_none(self):
        from app.core.safe_import import safe_import, clear_cache
        clear_cache()
        mod = safe_import("app.nonexistent.module.xyz")
        assert mod is None

    def test_safe_call_returns_default_on_error(self):
        from app.core.safe_import import safe_call, clear_cache
        clear_cache()
        result = safe_call("app.nonexistent.module", "some_func", default="fallback")
        assert result == "fallback"

    def test_safe_call_works_for_valid_func(self):
        from app.core.safe_import import safe_call
        result = safe_call("json", "dumps", {"key": "value"}, default=None)
