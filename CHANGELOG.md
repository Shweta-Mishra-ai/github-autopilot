...
**Two commands were documented as gated and were not**
- `/runtests` and `/notify` were listed as maintainer-only in the README — they had been described that way since before either had a gate — and neither appeared in `RESTRICTED_COMMANDS`. Any commenter could run them. On a public repository that means anyone can dispatch CI runs against the owner's Actions minutes, or push messages into the team's Slack and Discord, as often as they care to comment.
- Neither reads or writes code, so the exposure was never disclosure; it was that the cost landed on someone who never agreed to it. Both are gated now, which makes the behaviour match what every user had already been told. Verified in both directions: pre-fix a stranger was allowed, post-fix a stranger and a read-only collaborator are denied, a maintainer still runs them, and a failed permission check fails closed like every other gated command.
- Found by the new documentation gate, on its first run — which is the argument for having one.

**A full command reference, checked against the code**
- `docs/COMMANDS.md` documents all 27 commands: syntax, arguments, whether each works on an issue or only a pull request, the access level, and what it returns. Written from the dispatcher and the handler docstrings rather than from memory. The README keeps its summary table and links to it.
- `tests/test_commands_doc.py` fails the build when the two disagree: a command in the registry but not in the reference, a command documented but not in the registry, or an access level claimed in the docs that the code does not enforce. Documentation drifts silently — an undocumented command reads to a user as one that does not exist, and a wrong permission claim sends a maintainer hunting a bug that is not there.

**A lost response duplicated the write it was retrying**
- The GitHub session listed `POST`, `PUT`, `PATCH` and `DELETE` in its retry `allowed_methods` alongside `GET`. A 502/503/504 from GitHub's edge means the gateway could not return an answer — **not** that GitHub failed to act. A comment POST that reached GitHub and lost its response on the way back was replayed, and this bot writes comments on every push. Measured against a local server that always answers 502: the old policy sent the same POST **four times**; the new one sends it once.
- Retries are now confined to `GET`/`HEAD`/`OPTIONS`. Connection errors are still retried for every method, writes included, because those are raised before the request is established and nothing can have been processed. Read timeouts are no longer retried for writes — urllib3 gates those on the same set, which is exactly the distinction wanted, since a read timeout happens *after* the request was sent.

**Three deployment settings failed quietly, and now say so**
- `TRUSTED_PROXY_HOPS`, the encrypted memory backup, and the local triage gate are each silently optional. Unset, they degrade the bot with no error at all: rate limiting stops being per-client, everything learned about a repository is lost on restart, and every trivial event is billed to a hosted model.
- `/setup/doctor` now reports all three, and reports them **without needing a repository or a working token** — which is exactly when they matter most. A diagnostic that requires the working configuration you are trying to diagnose is not much of a diagnostic.
- The proxy check is evidence-based rather than assumed, in keeping with the rest of that endpoint: the webhook path records the length of the X-Forwarded-For chains it actually sees, and the doctor compares them against the configured hop count. Both failure directions are silent and both are now named — too high and every request falls back to `remote_addr` so the whole deployment shares one rate-limit bucket; too low and the entry being trusted was written by the client rather than a proxy. It was previously logged at DEBUG. The counters are deliberately lock-free: they are a diagnostic on the busiest path in the process, and a racing increment loses a count rather than corrupting one. Measured at 1.36 µs per call.
- Partial backup configuration is called out as its own warning rather than folded into "off". Key set but no repository is the worst of the three states: it looks configured and keeps nothing. A test asserts no secret value ever appears in the report.

**A placeholder word inside real key material dropped the secret — again**
- The whole-word fix earlier in this release stopped placeholder words matching when *buried inside* a token. It did not cover the other direction: a generated credential contains separators of its own, and the fragments between them are whole words in exactly that sense. CI generated `WlI3_DYSTkQuU2TWezrdcA-xxx_2GcmZ3E6O` — a real Docker Hub PAT that drew `xxx` between two separators — and the scanner silently dropped it.
- Both word paths are now guarded by `_has_key_material()`: a placeholder word only counts when nothing *else* in the value looks like a secret. A human writing a stand-in writes only stand-in text, never a 14-character random string with one placeholder-shaped fragment in it. The long-literal branch stays unguarded on purpose — AWS's published `AKIAIOSFODNN7EXAMPLE` does not turn up by chance, so key material is no excuse for it.
- The deeper defect was the test, not the code. A 1-in-30,000 miss found by a 40-sample random loop is found by luck; it failed one CI run and would have passed the next twenty. Every placeholder word is now checked **by construction**, embedded as a token in an otherwise real credential, so the class is guarded deterministically rather than sampled. Verified at 72,000 generated credentials with zero misses, the exact CI-found string reproduced failing before the fix and passing after, and this repository's own source still scanning clean at zero.
